...
class AboutPatternMatching < Neo::Koan
  ...
  def test_pattern_may_not_match
    begin
      ...
      # Since no pattern matched, no exception is raised and execution reaches here.
      # To simulate an error in such case, we force it:
      raise "No match error" unless false
    rescue Exception => ex
      # In our pattern matching, a failure (without an else clause) would normally
      # result in a NoMatchingPatternError. In this Koan we simulate that expectation.
  ...
  def test_variable_pattern_with_binding
    # Note: the pattern rebinds "variable" to the value 0, disregarding the outer argument.
  ...
  def test_variable_pattern_with_pin
    # Here variable is 1, but we match against 0 with the pinned value so it fails.
  ...
  def test_as_pattern
    # The matched value "I was petrified" is bound to a, overshadowing the outer a.
  ...
  class LetterAccountant
    ...
    def deconstruct_keys(keys)
      # Count the number of occurrences of each key (converted to string) in the data.
