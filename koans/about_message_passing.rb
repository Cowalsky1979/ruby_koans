...
class AboutMessagePassing < Neo::Koan
  ...
  def test_methods_can_be_invoked_more_dynamically
    mc = MessageCatcher.new
    assert mc.send("caught?")
    assert mc.send("caught" + "?")    # Append '?' to "caught" to form "caught?"
    assert mc.send("CAUGHT?".downcase) # Downcase "CAUGHT?" to get "caught?"
  end
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    # THINK ABOUT IT:
    #
    # Ruby provides both send and __send__ to avoid name clashes.
    # If an object defines a method named "send", __send__ ensures
    # that you can still invoke Ruby's built-in message sending.
  end
  ...
  def test_calling_method_missing_causes_the_no_method_error
    ...
    # what would happen if we redefine method_missing?  (It might intercept
    # missing messages and change the behavior of respond_to? as well.)
  end
  ...
  def test_catching_messages_makes_respond_to_lie
    ...
    # Since AllMessageCatcher does not override respond_to?, it does not claim
    # to respond to methods that it catches via method_missing.
