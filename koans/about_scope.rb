...
class AboutScope < Neo::Koan
  ...
  def test_bare_bones_class_names_assume_the_current_scope
    # Here, String refers to AboutScope::String (the nested one), so:
    assert_equal true, AboutScope::String == String
  end

  def test_nested_string_is_not_the_same_as_the_system_string
    # "HI".class is the built-in ::String, not AboutScope::String.
  ...
  def test_you_can_get_a_list_of_constants_for_any_class_or_module
    # Jims contains only the Dog constant.
    assert_equal [:Dog], Jims.constants
    # Object has many constants; 0 is a safe minimal bound.
